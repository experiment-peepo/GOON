...
namespace GOON.Tests {
    public class StandardHtmlFetcherTests {
        ...
        public async Task FetchHtmlAsync_WithCancellationToken_CanBeCancelled() {
            ...
            // Assert
            // Should either return null, return content (if completed before cancel check), or throw TaskCanceledException
            try {
                var result = await fetcher.FetchHtmlAsync(url, cts.Token);
                // If it doesn't throw, it's acceptable - either null or content if it completed quickly
