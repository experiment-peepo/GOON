...
public class YoutubeDL : PluginBase, IOpen, ISuggestExternalAudio, ISuggestExternalVideo
{
    ...
    public OpenResults Open()
    {
        try
        {
            lock (procLocker)
            {
                ...
                string arguments = $"{dynamicOptions}{Options["ExtraArguments"]} --no-check-certificate --skip-download --youtube-skip-dash-manifest --write-info-json -P \"{workingDir}\" \"{Playlist.Url}\" -o \"%(title).220B\"";

                // Inject Cookies if available
                if (!string.IsNullOrEmpty(Config.Demuxer.Cookies))
                {
                    try
                    {
                        var uri = new Uri(Playlist.Url);
                        string cookieFile = Path.Combine(workingDir, "cookies.txt");
                        CreateNetscapeCookieFile(cookieFile, Config.Demuxer.Cookies, uri.Host);
                        arguments += $" --cookies \"{cookieFile}\"";
                    }
                    catch (Exception ex) { Log.Warn($"Failed to setup extraction cookies: {ex.Message}"); }
                }

                // Inject User-Agent if available
                if (!string.IsNullOrEmpty(Config.Demuxer.UserAgent))
                {
                    arguments += $" --user-agent \"{Config.Demuxer.UserAgent}\"";
                }
    ...
    private void CreateNetscapeCookieFile(string path, string cookieString, string domain)
    {
