...
public static class Engine
{
    ...
    {
        // Skip Application creation if we're in a headless environment (e.g. tests)
        // or if Application already exists.
        if (Application.Current == null)
        {
            try
            {
                // Only try to create Application if we can (likely STA thread)
                        ...
                        foreach (var player in Players)
                        {
                            ...
                            if (curLoop == 0)
                            {
                                ...
                                if (config.Stats)
                                {
                                    ...
                                    player.Video.ClockDriftMs = player.Video.ClockDriftMs;
                                    player.Video.D3DImageLatencyMs = player.Video.D3DImageLatencyMs;
