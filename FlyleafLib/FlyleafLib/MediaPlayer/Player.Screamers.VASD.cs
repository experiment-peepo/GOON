...
unsafe partial class Player
{
    ...
    long startTicks, lastSpeedChangeTicks, startClockTicks;
        ...
        while (status == Status.Playing)
        {
            ...
            if (requiresBuffering)
            {
                ...
                if (vFrame == null)
                {
                    ...
                    {
                        Log.Warn("[Sync] Buffer Empty, waiting...");
                        Thread.Sleep(50);
                        continue;
                    }
                }
            ...
            if (Config.Player.MasterClock == MasterClock.External && ExternalClock != null)
            {
                ...
                vDistanceMs = (int)((vFrame.Timestamp - streamTime) / (10000 * speed));
                Video.ClockDriftMs = vDistanceMs;

                long toleranceMs = Config.Player.SyncTolerance / 10000;

                // Tiered Sync
                if (vDistanceMs < -500 || vDistanceMs > 5000) // Hard Resync
                {
                    Log.Warn($"[V] Hard Resync (Drift: {vDistanceMs}ms)");

                    if (vDistanceMs < -500 || vDistanceMs > 30000)
                    {
                        Log.Info($"[Sync] Proactive seek to {TicksToTimeMini(streamTime)}");
                        seeks.Push(new SeekData((int)(streamTime / 10000), false, false));
                    }

                    requiresBuffering = true;
                    continue;
                }
                else if (Math.Abs(vDistanceMs) > 1500) // Proactive micro-seek for External Clock (Threshold increased from 1000)
                {
                    Log.Info($"[Sync] Proactive micro-seek to {TicksToTimeMini(streamTime)} (Drift: {vDistanceMs}ms)");
                    seeks.Push(new SeekData((int)(streamTime / 10000), false, false));
                    requiresBuffering = true;
                    continue;
                }
                else if (Math.Abs(vDistanceMs) > 30) // Soft correction (Increased deadzone from 20ms to 30ms)
                {
                ...
                {
                    // Within deadzone, gradually return to ExternalClock speed
                    // Also dampened (0.002 instead of 0.05)
                    speed = VideoDecoder.Speed = AudioDecoder.Speed = (speed * 0.998) + (ExternalClock.Speed * 0.002);
                }
            }
            else
            {
            ...
            if (vDistanceMs < -3)
            {
                ...
                if (vDistanceMs < -500)// || GetBufferedDuration() < Config.Player.MinBufferDuration / 2)
                {
                    requiresBuffering = true;
                    continue;
                }
                ...
                do
                {
                    ...
                    if (Config.Player.MasterClock == MasterClock.External && ExternalClock != null)
                        vDistanceMs = (int)((vFrame.Timestamp - (ExternalClock.Ticks - startClockTicks + startTicks)) / (10000 * speed));
                    else
                        vDistanceMs = (int)((((vFrame.Timestamp - startTicks) / speed) - (long)(sw.ElapsedTicks * SWFREQ_TO_TICKS)) / 10000);
            ...
            if (Subtitles.isOpened)
            {
                ...
                bool isSFramePrevEnded;

                if (Config.Player.MasterClock == MasterClock.External && ExternalClock != null)
                {
                    sDistanceMs = sFrame != null ? (int)((sFrame.Timestamp - ExternalClock.Ticks) / 10000) : int.MaxValue;
                    isSFramePrevEnded = sFramePrev != null && (sFramePrev.Timestamp + (sFramePrev.duration * (long)10000)) - ExternalClock.Ticks < 0;
                }
                else
                {
                    elapsedTicks = (long)(sw.ElapsedTicks * SWFREQ_TO_TICKS);
                    sDistanceMs = sFrame != null ? (int)((((sFrame.Timestamp - startTicks) / speed) - elapsedTicks) / 10000) : int.MaxValue;
                    isSFramePrevEnded = sFramePrev != null && ((sFramePrev.Timestamp - startTicks + (sFramePrev.duration * (long)10000)) / speed) - elapsedTicks < 0;
                }

                if (isSFramePrevEnded)
                {
            ...
            if (Data.isOpened)
            {
                ...
                if (Config.Player.MasterClock == MasterClock.External && ExternalClock != null)
                    dDistanceMs = dFrame != null ? (int)((dFrame.Timestamp - ExternalClock.Ticks) / 10000) : int.MaxValue;
                else
                {
                    elapsedTicks = (long)(sw.ElapsedTicks * SWFREQ_TO_TICKS);
                    dDistanceMs = dFrame != null ? (int)((((dFrame.Timestamp - startTicks) / speed) - elapsedTicks) / 10000) : int.MaxValue;
                }
